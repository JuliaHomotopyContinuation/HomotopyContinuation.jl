...
* `threading = true`: Enable multi-threading for the computation. The number of available threads is controlled by the environment variable `JULIA_NUM_THREADS`. (Some CPUs hang when using multiple threads. To avoid this run Julia with 1 interactive thread for the REPL and `n` threads for other tasks; e.g., `julia -t 8,1` for `n=8`. On the other hand, some other CPUs crash when using an interactive thread. In this case, run `Julia` as `julia -t 8`)
