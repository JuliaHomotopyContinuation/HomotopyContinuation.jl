...
function taylor_op_cb(V::Val{K}, x::TruncatedTaylorSeries{M}) where {K, M}
    return taylor_op_mul(V, taylor_op_sqr(V, x), x)
end
# OP_COS and OP_SIN
...
function taylor_op_identity(::Val{K}, x::TruncatedTaylorSeries{M, T}) where {K, M, T}
    return convert(TruncatedTaylorSeries{K + 1, T}, x)
end
#
# OP_ADD # a + b
...
function taylor_op_pow_int_impl(K, dx)
    ...
    return quote
        Base.@_inline_meta
        $(untuple(:x, dx))
        iszero(x0) && return $(taylor_tuple([nothing for _ in 0:K]))
        w₀ = op_pow_int(x0, r)
...
function taylor_op_add3(
    ...
    return taylor_op_add(V, taylor_op_add(V, x, y), z)
end
# OP_MUL3 # a * b * c
function taylor_op_mul3(
    ...
    return taylor_op_mul(V, taylor_op_mul(V, x, y), z)
end
# OP_MULADD # a * b + c
@generated function taylor_op_muladd(
    ...
    return taylor_impl(K, M - 1, N - 1, L - 1) do list, D
        map(0:K) do k
            c_k = D[:z, k]
            for j in 0:k
                c_k = muladd!(list, D[:x, j], D[:y, k - j], c_k)
...
function taylor_op_add4(
    ...
    return taylor_op_add(V, taylor_op_add(V, x, y), taylor_op_add(V, z, w))
end
# OP_MUL4 # a * b * c * d
function taylor_op_mul4(
    ...
    return taylor_op_mul(V, taylor_op_mul(V, x, y), taylor_op_mul(V, z, w))
end
...
function taylor_op_mulmuladd(
    ...
    return taylor_op_add(V, taylor_op_mul(V, x, y), taylor_op_mul(V, z, w))
end
...
function taylor_op_mulmulsub(
    ...
    return taylor_op_sub(V, taylor_op_mul(V, x, y), taylor_op_mul(V, z, w))
end
