...
Given a homotopy ``H(x,t): (ℙᵐ¹ × ⋯ × ℙᵐᴺ) × ℂ → ℂⁿ`` this creates a new affine homotopy ``H̄``
which operates on the affine chart defined by the vector ``v ∈ ℙᵐ¹ × ⋯ × ℙᵐᴺ`` and the
augmented conditions `` vᵀx = 1 ``.
"""
