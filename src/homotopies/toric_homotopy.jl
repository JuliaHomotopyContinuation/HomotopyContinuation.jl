...
function evaluate_weights!(u::Vector{Float64}, weights::Vector{Float64}, t::Float64)
    s = log(t)
    n = length(weights)
    LoopVectorization.@avx for i in 1:n
        u[i] = exp(weights[i] * s)
    end
    return u
end
...
function evaluate_weights!(
    ...
    n = length(weights)
    sr, si = reim(s)
    return LoopVectorization.@avx for i in 1:n
        r = exp(weights[i] * sr)
        wsi = weights[i] * si
...
function dt_coeffs!(H::ToricHomotopy, t)
    if iszero(t)
        @inbounds for i in 1:length(H.dt_coeffs)
            wᵢ = H.weights[i]
            uᵢ = H.system_coeffs[i]
...
function ModelKit.evaluate!(u, H::ToricHomotopy, x::AbstractVector, t)
    return evaluate!(u, H.system, x, coeffs!(H, t))
end
...
function ModelKit.taylor!(u, v::Val{1}, H::ToricHomotopy, x::Vector, t)
    return evaluate!(u, H.system, x, dt_coeffs!(H, t))
end
...
function ModelKit.taylor!(u, v::Val{2}, H::ToricHomotopy, tx::TaylorVector, t)
    ...
    return ModelKit.taylor!(u, v, H.system, tx, H.tc2)
end
function ModelKit.taylor!(u, v::Val{3}, H::ToricHomotopy, tx::TaylorVector, t)
    taylor_coeffs!(H, real(t))
    return ModelKit.taylor!(u, v, H.system, tx, H.tc3)
end
function ModelKit.taylor!(u, v::Val{4}, H::ToricHomotopy, tx::TaylorVector, t)
    taylor_coeffs!(H, real(t))
    return ModelKit.taylor!(u, v, H.system, tx, H.taylor_coeffs)
end
