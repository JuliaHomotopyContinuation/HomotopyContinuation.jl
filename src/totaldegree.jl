...
MultiBezoutIndicesIterator(D::Matrix, groups::VariableGroups) =
    MultiBezoutIndicesIterator(D, projective_dims(groups))
...
Base.length(iter::MultiBezoutSolutionsIterator) =
    bezout_number(iter.indices.D, iter.indices.k)
Base.IteratorSize(::Type{<:MultiBezoutSolutionsIterator}) = Base.SizeUnknown()
Base.eltype(::Type{MultiBezoutSolutionsIterator{N}}) where {N} =
    ProjectiveVectors.PVector{ComplexF64,N}
