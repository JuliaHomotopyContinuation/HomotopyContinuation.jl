export path_info

struct EndgamePathInfo
    # per step info
    s::Vector{Float64}
    Δs::Vector{Float64}
    ω::Vector{Float64}
    Δx₀::Vector{Float64}
    accepted_rejected::Vector{Bool}
    norm_x::Vector{Float64}
    accuracy::Vector{Float64}
    μ::Vector{Float64}
    τ::Vector{Float64}
    Δx_t::Vector{Float64}
    high_prec::Vector{Bool}
    dist_sol::Vector{Float64}
    rconds::Vector{Float64}
    # total info
    return_code::EndgameCode.codes
    n_factorizations::Int
    n_ldivs::Int
end

"""
    path_info(tracker::Tracker, x₀, t₁ = 1.0, t₀ = 0.0; debug::Bool = false, kwargs...)

Track a path using the given `tracker` and start value `x₀`.
This returns a struct containing detailed information about the tracked path.
"""
function path_info(
    eg::EndgameTracker,
    x₀,
    t₁ = 1.0,
    t₀ = 0.0;
    debug::Bool = false,
    kwargs...,
)
    tracker = eg.tracker
    state = tracker.state

    s = Float64[]
    Δs = Float64[]
    ω = Float64[]
    Δx₀ = Float64[]
    accepted_rejected = Bool[]
    norm_x = Float64[]
    accuracy = Float64[]
    μ = Float64[]
    τ = Float64[]
    Δx_t = Float64[]
    high_prec = Bool[]
    dist_sol = Float64[]
    rconds = Float64[]

    p = order(tracker.predictor)

    init!(eg, x₀, t₁, t₀; kwargs...)
    while is_tracking(eg.state.code)
        println("t = ", real(state.t))
        push!(s, real(state.t))
        push!(Δs, real(state.Δt))

        push!(τ, trust_region(tracker.predictor))
        push!(ω, state.ω)
        push!(accuracy, state.accuracy)
        push!(μ, state.μ)
        push!(high_prec, state.extended_prec)
        push!(
            dist_sol,
            real(state.t) * (
                LA.norm(eg.state.projective_path_value.ż) +
                eg.state.target_system_normalized_norm_derivative
            ),
        )
        push!(rconds, 1 / rcond!(eg.tracker.ws))
        step!(eg)

        println("val = ", eg.state.valuation)

        push!(Δx_t, abs(Δs[end]) / state.norm_Δx₀^(1 / p))

        push!(accepted_rejected, !state.last_step_failed)
        push!(Δx₀, state.norm_Δx₀)
        push!(norm_x, maximum(abs, state.x))
    end

    EndgamePathInfo(
        s,
        Δs,
        ω,
        Δx₀,
        accepted_rejected,
        norm_x,
        accuracy,
        μ,
        τ,
        Δx_t,
        high_prec,
        dist_sol,
        rconds,
        eg.state.code,
        tracker.ws.statistics.factorizations[],
        tracker.ws.statistics.solves[],
    )
end

path_table(info::EndgamePathInfo) = path_table(stdout, info)
function path_table(io::IO, info::EndgamePathInfo)
    header = ["", "s", "Δs", "ω", "|Δx₀|", "acc", "μ", "τ", "|x|", "dist_sol", "κ"]
    h1 = PrettyTables.Highlighter(
        f = (data, i, j) -> j == 1 && data[i, 1] == :✗,
        crayon = PrettyTables.crayon"red",
    )
    h2 = PrettyTables.Highlighter(
        f = (data, i, j) -> j == 1 && data[i, 1] == :✓,
        crayon = PrettyTables.crayon"green",
    )
    # h3 = PrettyTables.Highlighter(
    #     f=(data, i, j) -> j == 3 && data[i, 3] ≈ -data[i, 9],
    #     crayon=PrettyTables.crayon"blue",
    # )
    h4 = PrettyTables.Highlighter(
        f = (data, i, j) -> j == 7 && info.high_prec[i],
        crayon = PrettyTables.crayon"blue",
    )

    ✓✗ = map(v -> v ? :✓ : :✗, info.accepted_rejected)
    data = hcat(
        ✓✗,
        info.s,
        info.Δs,
        info.ω,
        info.Δx₀,
        info.accuracy,
        info.μ,
        info.τ,
        info.norm_x,
        info.dist_sol,
        info.rconds,
    )

    ft1 = PrettyTables.ft_printf("%5.2g", [4, 5, 6, 7, 8, 10, 11, 12])
    ft2 = PrettyTables.ft_printf("%5.2g", [3, 9])
    ft4 = PrettyTables.ft_printf("%3.3g", [2, 4])

    PrettyTables.pretty_table(
        io,
        data;
        header = header,
        crop = :none,
        formatters = (ft1, ft2, ft4),
        highlighters = (h1, h2, h4),
    )
end

function Base.show(io::IO, info::EndgamePathInfo)
    println(io, "EndgamePathInfo:")
    println(io, " • # return code → ", info.return_code)
    println(
        io,
        " • # steps (✓/✗) → ",
        length(info.s),
        " ( ",
        count(info.accepted_rejected),
        " / ",
        count(!, info.accepted_rejected),
        " )",
    )
    println(io, " • # factorizations → ", info.n_factorizations)
    println(io, " • # ldivs → ", info.n_ldivs)
    path_table(io, info)
end
