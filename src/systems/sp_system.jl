...
SPSystem(polys::Vector{<:MP.AbstractPolynomial}, vars) =
    SPSystem(SP.PolynomialSystem(polys, variables = vars))
SPSystem(polys::Vector{<:MP.AbstractPolynomial}; kwargs...) =
    SPSystem(SP.PolynomialSystem(polys; kwargs...))
function SPSystem(exponents::Vector{<:Matrix}, coeffs::Vector{<:Vector})
    n = length(exponents)
    @assert n == length(coeffs)
    SPSystem(SP.PolynomialSystem((SP.Polynomial(coeffs[i], exponents[i]) for i = 1:n)...))
end

Base.size(F::SPSystem) = (SP.npolynomials(F.system), SP.nvariables(F.system))

cache(F::SPSystem, x, p = nothing) = SystemNullCache()
@propagate_inbounds evaluate!(u, F::SPSystem, x, ::SystemNullCache) =
    SP.evaluate!(u, F.system, x)
@propagate_inbounds evaluate!(u, F::SPSystem, x, p, ::SystemNullCache) =
    SP.evaluate!(u, F.system, x, p)
...
@propagate_inbounds jacobian!(U, F::SPSystem, x, ::SystemNullCache) =
    SP.jacobian!(U, F.system, x)
@propagate_inbounds jacobian!(U, F::SPSystem, x, p, ::SystemNullCache) =
    SP.jacobian!(U, F.system, x, p)
...
@propagate_inbounds evaluate_and_jacobian!(u, U, F::SPSystem, x, ::SystemNullCache) =
    SP.evaluate_and_jacobian!(u, U, F.system, x)
@propagate_inbounds evaluate_and_jacobian!(u, U, F::SPSystem, x, p, ::SystemNullCache) =
    SP.evaluate_and_jacobian!(u, U, F.system, x, p)
evaluate_and_jacobian(F::SPSystem, x, ::SystemNullCache) =
    SP.evaluate_and_jacobian(F.system, x)
evaluate_and_jacobian(F::SPSystem, x, p, ::SystemNullCache) =
    SP.evaluate_and_jacobian(F.system, x, p)
@propagate_inbounds differentiate_parameters!(U, F::SPSystem, x, p, ::SystemNullCache) =
    SP.differentiate_parameters!(U, F.system, x, p)
differentiate_parameters(F::SPSystem, x, p, ::SystemNullCache) =
    SP.differentiate_parameters(F.system, x, p)
...
"""
@generated function set_coefficients!(
    G::SP.PolynomialSystem{N},
