...
function validate_result(BSS)
    for k in 1:size(BSS.X, 2)
        for j in 1:size(BSS.A, 2)
            ...
            if fast_abs(r) > max(fast_abs(BSS.b[j]) * 1.0e-8, 1.0e-8)
                return false
            end
        end
    end
