...
function Result(ri::ResultIterator)
    ...
    return Result(C; seed = ri.S.seed, start_system = ri.S.start_system)
end
...
Get all results where the solutions are real.
...
larger than 1 or the condition number is large.
