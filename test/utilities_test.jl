@testset "Utilities" begin
    ...
    @testset "Linear algebra" begin
        ...
        C, d = copy(A), copy(b)
        HC.solve!(x, jac, b)
        @test norm(x - C \ d) < 1e-10

        # square QR / update infos (full rank)
        HC.update!(jac, A; update_infos=true)
        @test jac.corank == 0
        @test jac.cond > 1
        HC.solve!(x, jac, b; update_digits_lost=false)
        @test norm(x - C \ d) < 1e-10

        x .= 0
        HC.solve!(x, jac, b; update_digits_lost=true)
        @test norm(x - C \ d) < 1e-10

        # square QR / not full rank
        A = rand(Complex{Float64}, 12, 12)
        b = rand(Complex{Float64}, 12)
        x = zeros(ComplexF64, 12)
        jac = HC.Jacobian(A)

        # make a corank 1 matrix
        U, s, V = svd(A)
        s[end] = 1e-16
        Â = U * diagm(0 => s) * V'
        ...
        @test norm(x - qr(Â ./ jac.D, Val(true)) \ (b ./ jac.D)) < 1e-10

        x .= 0
        HC.solve!(x, jac, copy(b); update_digits_lost=true)
        @test norm(x - qr(Â ./ jac.D, Val(true)) \ (b ./ jac.D)) < 1e-10
        @test jac.digits_lost == nothing
        ...
        x = zeros(ComplexF64, 12)
        HC.solve!(x, jac, b)
        @test norm(x - qr(C, Val(true)) \ d) < 1e-10
