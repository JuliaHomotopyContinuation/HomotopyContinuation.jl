function RigidMultiView()
    ...
    ũ = [A[:, :, k] * [x[1:3]; 1] for k in 1:2]
    û = [ũ[k][1:2] ./ ũ[k][3] for k in 1:2]
    ...
    ũ = [A[:, :, k] * [x[1:3]; 1] for k in 1:2]
    û = [ũ[k][1:2] ./ ũ[k][3] for k in 1:2]
    ṽ = [A[:, :, k] * [y[1:3]; 1] for k in 1:2]
    v̂ = [ṽ[k][1:2] ./ ṽ[k][3] for k in 1:2]
    ...
    return System([differentiate(L, [x; y]); r]; parameters = [u; v; δ; vec(A)])
end
...
function Tritangtens()
    ...
    #define F
    return F = System([P_x; P_y; P_z]; variables = [h; x; y; z], parameters = c)
end
...
function cyclo()
    ...
    return f = System([F1; F2; F3; F4])
end


function cyclic(n)
    @var z[1:n]
    eqs = [sum(prod(z[(k - 1) % n + 1] for k in j:(j + m)) for j in 1:n) for m in 0:(n - 2)]
    push!(eqs, prod(z) - 1)
    return System(eqs, z)
end
...
function bacillus()
    ...
    return System(poly, [w, w2, w2v, v, w2v2, vP, sigmaB, w2sigmaB, vPp, phos])
end
...
function six_revolute()
    ...
    f = [z[i, :] ⋅ z[i, :] for i in 2:5]
    g = [z[i, :] ⋅ z[i + 1, :] for i in 1:5]
    h = sum(a[i] .* (z[i, :] × z[i + 1, :]) for i in 1:3) + sum(a[i + 4] .* z[i, :] for i in 2:5)
    F′ = [f .- 1; g .- cos.(α); h .- p]
...
function fano_quintic()
    ...
    return System(ModelKit.horner.(FcapL), [a; b], q)
end
...
function moments3()
    ...
    return System([f0, f1, f2, f3, f4, f5, f6, f7, f8] - m, parameters = m)
end
...
function sqrt_parameters()
    ...
    return System([sqrt(a + b) * x^2 - y, (x * y + a - sqrt(b))^2 - 3], [x, y], [a, b])
end
