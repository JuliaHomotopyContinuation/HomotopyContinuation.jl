@testset "nextjournal" begin
    ...
    @testset "Groups of variables" begin
        @polyvar x y z
        J = 3 * x^3 * y + y^2 * z^2 - 2x * y - x * 4 * z^3
        g = x^4 + y^4 + z^4 - 1
    ...
    @testset "Monodromy Method" begin
        ...
        m3 = a[1] * (μ[1]^3 + 3 * σ²[1] * μ[1]) + a[2] * (μ[2]^3 + 3 * σ²[2] * μ[2]) +
             a[3] * (μ[3]^3 + 3 * σ²[3] * μ[3])
        m4 = a[1] * (μ[1]^4 + 6 * σ²[1] * μ[1]^2 + 3 * σ²[1]^2) +
             a[2] * (μ[2]^4 + 6 * σ²[2] * μ[2]^2 + 3 * σ²[2]^2) +
        ...
        m7 = a[1] * (μ[1]^7 + 21 * σ²[1] * μ[1]^5 + 105 * μ[1]^3 * σ²[1]^2 +
              105 * μ[1] * σ²[1]^3) +
             a[2] * (μ[2]^7 + 21 * σ²[2] * μ[2]^5 + 105 * μ[2]^3 * σ²[2]^2 +
              105 * μ[2] * σ²[2]^3) +
             a[3] *
             (μ[3]^7 + 21 * σ²[3] * μ[3]^5 + 105 * μ[3]^3 * σ²[3]^2 + 105 * μ[3] * σ²[3]^3)
        m8 = a[1] * (μ[1]^8 + 28 * σ²[1] * μ[1]^6 + 210 * μ[1]^4 * σ²[1]^2 +
              420 * μ[1]^2 * σ²[1]^3 + 105 * σ²[1]^4) +
             a[2] * (μ[2]^8 + 28 * σ²[2] * μ[2]^6 + 210 * μ[2]^4 * σ²[2]^2 +
              420 * μ[2]^2 * σ²[2]^3 + 105 * σ²[2]^4) +
             a[3] * (μ[3]^8 + 28 * σ²[3] * μ[3]^6 + 210 * μ[3]^4 * σ²[3]^2 +
              420 * μ[3]^2 * σ²[3]^3 + 105 * σ²[3]^4)
        ...
        S₃ = SymmetricGroup(3)
        relabeling = GroupActions(v -> map(
            p -> (v[1:3][p]..., v[4:6][p]..., v[7:9][p]...),
            S₃,
        ))
        mults = multiplicities(true_real_solutions, group_action = relabeling)
        @test length(mults) ≥ 1

        R_with_group_action = monodromy_solve(
            f - p,
            start_sol,
            q₀;
            parameters = p,
