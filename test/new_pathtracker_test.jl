...
using HomotopyContinuation, HomotopyContinuationGym, Test
using LinearAlgebra
const HC = HomotopyContinuation
import PolynomialTestSystems: equations, katsura, griewank_osborne, cyclic
import PolynomialTestSystems, ProjectiveVectors
...
@testset "PathTracker" begin
    ...
    @testset "Simple" begin
        ...
        for x in starts
            @test is_success(track!(tracker, x))
            s = ProjectiveVectors.affine_chart(solution(tracker))
            @test abs(s[1]) ≈ sqrt(2) atol = 1e-8
            @test sum(s) ≈ 1 atol = 1e-8
            @test isnothing(winding_number(tracker))
        end
